...
public class CharLineStreamReader : TextReader
{
    // CharLineStreamReader.Null is threadsafe.
    public static new readonly CharLineStreamReader Null = new NullCharLineStreamReader();
    ...
    private Encoding _encoding = null!; // only null in NullCharLineStreamReader where this is never used
    private Decoder _decoder = null!; // only null in NullCharLineStreamReader where this is never used
    private readonly byte[] _byteBuffer = null!; // only null in NullCharLineStreamReader where this is never used
    private char[] _charBuffer = null!; // only null in NullCharLineStreamReader where this is never used
    ...
    // disposing of this CharLineStreamReader.  A name like _leaveOpen is better,
    ...
    // We don't guarantee thread safety on CharLineStreamReader, but we should at
    ...
    // CharLineStreamReader by default will ignore illegal UTF8 characters. We don't want to
    // throw here because we want to be able to read ill-formed data without choking.
    // The high level goal is to be tolerant of encoding errors when we read and very strict
    // when we write. Hence, default StreamWriter encoding will throw on error.

    private CharLineStreamReader()
    {
        Debug.Assert(this is NullCharLineStreamReader);
        _stream = Stream.Null;
        _closable = true;
    }

    public CharLineStreamReader(Stream stream)
        : this(stream, true)
    {
    }

    public CharLineStreamReader(Stream stream, bool detectEncodingFromByteOrderMarks)
        : this(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
    {
    }

    public CharLineStreamReader(Stream stream, Encoding encoding)
        : this(stream, encoding, true, DefaultBufferSize, false)
    {
    }

    public CharLineStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        : this(stream, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize, false)
    {
    }

    // Creates a new CharLineStreamReader for the given stream.  The
    ...
    //
    public CharLineStreamReader(Stream stream, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
        : this(stream, encoding, detectEncodingFromByteOrderMarks, bufferSize, false)
    {
    }

    public CharLineStreamReader(Stream stream, Encoding? encoding = null, bool detectEncodingFromByteOrderMarks = true, int bufferSize = -1, bool leaveOpen = false)
    {
    ...
    public CharLineStreamReader(string path)
        : this(path, true)
    {
    }

    public CharLineStreamReader(string path, bool detectEncodingFromByteOrderMarks)
        : this(path, Encoding.UTF8, detectEncodingFromByteOrderMarks, DefaultBufferSize)
    {
    }

    public CharLineStreamReader(string path, Encoding encoding)
        : this(path, encoding, true, DefaultBufferSize)
    {
    }

    public CharLineStreamReader(string path, Encoding encoding, bool detectEncodingFromByteOrderMarks)
        : this(path, encoding, detectEncodingFromByteOrderMarks, DefaultBufferSize)
    {
    }

    public CharLineStreamReader(string path, Encoding encoding, bool detectEncodingFromByteOrderMarks, int bufferSize)
    ...
    public CharLineStreamReader(string path, FileStreamOptions options)
        : this(path, Encoding.UTF8, true, options)
    {
    }

    public CharLineStreamReader(string path, Encoding encoding, bool detectEncodingFromByteOrderMarks, FileStreamOptions options)
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        // Dispose of our resources if this CharLineStreamReader is closable.
        if (_closable)
        {
            try
            {
    ...
    // DiscardBufferedData tells CharLineStreamReader to throw away its internal
    // buffer contents.  This is useful if the user needs to seek on the
    // underlying stream to a known location then wants the CharLineStreamReader
    // to start reading from this new point.  This method should be called
    // very sparingly, if ever, since it can lead to very poor performance.
    // However, it may be the only way of handling some scenarios where
    // users need to re-read the contents of a CharLineStreamReader a second time.
    public void DiscardBufferedData()
    {
        ...
        // for startup improvements for NullCharLineStreamReader, we want to delay load encoding.
        if (_encoding != null)
        {
    ...
    public override int Read(Span<char> buffer) =>
        GetType() == typeof(CharLineStreamReader) ? ReadSpan(buffer) :
    ...
    {
        if (GetType() != typeof(CharLineStreamReader))
        {
    ...
    {
        Debug.Assert(_byteLen >= n, "CompressBuffer was called with a number of bytes greater than the current buffer length.  Are two threads using this CharLineStreamReader at the same time?");
    ...
    private bool IsPreamble()
    {
        ...
        bool IsPreambleWorker()
        {
            ...
            Debug.Assert(_bytePos < preamble.Length, "_compressPreamble was called with the current bytePos greater than the preamble buffer length.  Are two threads using this CharLineStreamReader at the same time?");
            ...
            Debug.Assert(_bytePos <= preamble.Length, "possible bug in _compressPreamble.  Are two threads using this CharLineStreamReader at the same time?");
    ...
    internal virtual int ReadBuffer()
    {
        ...
        do
        {
            if (_checkPreamble)
            {
                Debug.Assert(_bytePos <= _encoding.Preamble.Length, "possible bug in _compressPreamble.  Are two threads using this CharLineStreamReader at the same time?");
            ...
            {
                Debug.Assert(_bytePos == 0, "bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this CharLineStreamReader at the same time?");
    ...
    // user's buffer, bypassing CharLineStreamReader's own buffer.
    ...
    private int ReadBuffer(Span<char> userBuffer, out bool readToUserBuffer)
    {
        ...
        do
        {
            ...
            else
            {
                ...
                _charLen = charsRead;  // Number of chars in CharLineStreamReader's buffer.
            }
        } while (charsRead == 0);
...
#region JP CODE - ReadLineToCharArray
    public char[]? ReadLineToCharArray()
    {
        ThrowIfDisposed();
        CheckAsyncTaskInProgress();
        ...
        do
        {
            ...
            if (idxOfNewline >= 0)
            {
                ...
                if (vsb.Length == 0)
                {
                    retVal = charBufferSpan.Slice(0, idxOfNewline).ToArray();
                }
                else {
                    vsb.Append(charBufferSpan.Slice(0, idxOfNewline));
                    retVal = vsb.AsSpan().ToArray();
                    vsb.Dispose();
                }
    ...
    /// using CharLineStreamReader reader = File.OpenText("existingfile.txt");
    ///
    /// string line;
    ...
    /// <see cref="CharLineStreamReader"/>) or returned (to the caller) may be lost.
    /// </remarks>
        ...
        while (count > 0)
        {
            ...
            if (n == 0)
            {
                ...
                do
                {
                        ...
                        if (len == 0)
                        {
                            ...
                            if (_byteLen > 0)
                            {
                                if (readToUserBuffer)
                                {
                                    ...
                                    _charLen = 0;  // CharLineStreamReader's buffer is empty.
                                }
                                else
                                {
                                    n = _decoder.GetChars(tmpByteBuffer, 0, _byteLen, _charBuffer, 0);
                                    _charLen += n;  // Number of chars in CharLineStreamReader's buffer.
                                }
                            }
                    ...
                    {
                        Debug.Assert(_bytePos == 0, "_bytePos can be non zero only when we are trying to _checkPreamble.  Are two threads using this CharLineStreamReader at the same time?");
    ...
    internal sealed class NullCharLineStreamReader : CharLineStreamReader
    {
